...
/* ──────────────────────────────────────────────────────
 *  Hybrid-physics SensorManager with
 *  – startStreaming() / stopStreaming() **unchanged API**
 *  – New, stabler DY physics and refined DX mapping
 * ────────────────────────────────────────────────────── */
class PhysicsModelSensorManager(
    context: Context,
    private val coroutineScope: CoroutineScope
    ...
    companion object {
        ...
        private const val DT = PACKET_INTERVAL_MS / 1000f    // seconds

        /* Gyroscope ➜ ΔX  (yaw) */
        private const val GYRO_SENSITIVITY = 900f            // pixels / rad s⁻¹
        private const val GYRO_ALPHA = 0.20f                 // low-pass weight
        private const val GYRO_RESPONSE_POW = 1.7f
        private const val GYRO_CAL_SAMPLES = 100
        private const val GYRO_CAL_MAX_VAR = 1e-5f

        /* Linear accel ➜ ΔY  (for/back) */
        private const val ACC_SENSITIVITY = 1200f            // pixels / ( m s⁻¹ )
        private const val DEADZONE = 0.05f                   // g-noise ignore
        private const val PASSIVE_FRICTION = 0.99f
        private const val BRAKE_FRICTION = 0.90f
        private const val V_SNAP = 0.001f
        private const val ACC_CAL_SAMPLES = 150
        private const val ACC_CAL_MAX_VAR = 0.001f
    }
    ...
    /* ─────  LINEAR ACCEL (ΔY) STATE  ────────────────── */
    private var velY = 0f
    private var accDriftY = 0f
    private val accBuf = mutableListOf<Float>()
    @Volatile private var currentDy = 0
    @Volatile private var latestAccY = 0f

    /* ──────────────────────────────────────────────────
     *  PUBLIC CONTROL  – identical names to original code
     * ────────────────────────────────────────────────── */
    fun startStreaming() {
        ...
        if (!sensorsRegistered) {
            ...
            sensorMgr.registerListener(this, linAccSensor,   rate)
            sensorsRegistered = true
        }

        /* Fixed-interval coroutine drives both physics and outbound messages */
        streamJob = coroutineScope.launch {
            while (isActive) {
                physicsTick()

                val move = Movement(currentDx, currentDy)
                try {
                    ConnectionManager.sendMessage(
                        movementToJson(move.dx, move.dy)
                    )
                } catch (e: Exception) {
    ...
    override fun onSensorChanged(event: SensorEvent) = when (event.sensor.type) {
        Sensor.TYPE_GYROSCOPE -> handleGyro(event.values[2])          // Z-axis → yaw
        Sensor.TYPE_LINEAR_ACCELERATION -> handleLinAcc(event.values[1]) // Y-axis
        else -> Unit
    }
    ...
    /* ──────────────────────────────────────────────────
     *  PHYSICS  – single step executed at fixed cadence
     * ────────────────────────────────────────────────── */
    private fun physicsTick() {
        /* 1 ▸ integrate acceleration → velocity */
        velY += latestAccY * DT

        /* 2 ▸ friction model */
        val accelActive = abs(latestAccY) > 0f
        val braking = velY != 0f && sign(latestAccY) != sign(velY) && !accelActive
        velY *= when {
            braking      -> BRAKE_FRICTION
            !accelActive -> PASSIVE_FRICTION
            else         -> 1f
        }

        /* 3 ▸ snap micro-drift */
        if (!accelActive && abs(velY) < V_SNAP) velY = 0f

        /* 4 ▸ convert to pixel deltas */
        currentDy = (-velY * ACC_SENSITIVITY).toInt()
    }

    /* ──────────────────────────────────────────────────
     *  GYROSCOPE → ΔX
     * ────────────────────────────────────────────────── */
    private fun handleGyro(raw: Float) {
        /* drift-learning buffer */
        yawBuf += raw
        if (yawBuf.size >= GYRO_CAL_SAMPLES) {
            if (yawBuf.variance() < GYRO_CAL_MAX_VAR)
                yawDrift = yawBuf.average().toFloat()
            yawBuf.clear()
        }

        val corrected = raw - yawDrift
        smoothYaw = GYRO_ALPHA * corrected + (1 - GYRO_ALPHA) * smoothYaw
        val curved = smoothYaw.signPow(GYRO_RESPONSE_POW)

        currentDx = (-curved * GYRO_SENSITIVITY).toInt()
    }

    /* ──────────────────────────────────────────────────
     *  LINEAR-ACCEL → latestAccY
     * ────────────────────────────────────────────────── */
    private fun handleLinAcc(raw: Float) {
        accBuf += raw
        if (accBuf.size >= ACC_CAL_SAMPLES) {
            if (accBuf.variance() < ACC_CAL_MAX_VAR && abs(velY) < V_SNAP)
                accDriftY = accBuf.average().toFloat()
            accBuf.clear()
        }

        val corrected = raw - accDriftY
        latestAccY = if (abs(corrected) > DEADZONE) corrected else 0f
    }
    ...
    private fun resetState() {
        ...
        velY = 0f; accDriftY = 0f; accBuf.clear(); currentDy = 0; latestAccY = 0f
    }
    ...
    private fun Float.signPow(p: Float): Float =
        sign(this) * abs(this).pow(p)
